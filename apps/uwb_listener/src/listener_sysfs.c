...
static struct listener_sysfs_data listener_sysfs_inst[MYNEWT_VAL(UWB_DEVICE_MAX)] = {0};


static ssize_t listener_show(struct device *dev,
...
static ssize_t listener_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    int rc;
    long param;
    struct dev_ext_attribute *ext_dev = (struct dev_ext_attribute *)attr;
    struct listener_sysfs_data *ed = (struct listener_sysfs_data *)ext_dev->var;
...
#define DEV_ATTR_ROW(__X, __Y)                                          \
    (struct dev_ext_attribute){                                         \
        .attr = {.attr = {.name = __X,                                  \
                 ...
                 .show = listener_show, .store = listener_store},                 \
            .var=__Y }

void
uwb_listener_sysfs_init(struct uwb_listener_instance *listener)
{
    int i;
    int rc;
    struct listener_sysfs_data *ed;
    if (listener->dev_inst->idx >= ARRAY_SIZE(listener_sysfs_inst)) {
        return;
    }
    ed = &listener_sysfs_inst[listener->dev_inst->idx];
    ed->listener = listener;

    mutex_init(&ed->local_lock);

    snprintf(ed->device_name, sizeof(ed->device_name)-1, "uwb_listener%d", listener->dev_inst->idx);

    ed->kobj = kobject_create_and_add(ed->device_name, kernel_kobj);
    if (!ed->kobj) {
        slog("Failed to create uwb_listener_kobj\n");
        return;
    }
...
{
    struct listener_sysfs_data *ed;
    if (idx >= ARRAY_SIZE(listener_sysfs_inst)) {
        return;
    }
    ed = &listener_sysfs_inst[idx];
