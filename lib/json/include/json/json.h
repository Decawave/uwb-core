...
#define JSON_VALUE_TYPE_FLOAT64 (6)
...
#define JSON_VALUE_FLOAT64(__jv, __v)          \
    (__jv)->jv_type = JSON_VALUE_TYPE_FLOAT64; \
    (__jv)->jv_val.fl = (dpl_float64_t) __v;
...
#define json_errorstr(rc)   ((rc==JSON_ERR_OBSTART)?"non-WS when expecting object start": \
                            (rc==JSON_ERR_ATTRSTART)?"non-WS when expecting attrib start":\
                            (rc==JSON_ERR_BADATTR)?"unknown attribute name": \
                            (rc==JSON_ERR_ATTRLEN)?"attribute name too long": \
                            (rc==JSON_ERR_NOARRAY)?"saw [ when not expecting array": \
                            (rc==JSON_ERR_NOBRAK)?"array element specified, but no [": \
                            (rc==JSON_ERR_STRLONG)?"string value too long": \
                            (rc==JSON_ERR_TOKLONG)?"token value too long": \
                            (rc==JSON_ERR_BADTRAIL)?"garbage while expecting comma or } or ]": \
                            (rc==JSON_ERR_ARRAYSTART)?"didn't find expected array start": \
                            (rc==JSON_ERR_OBJARR)?"error while parsing object array": \
                            (rc==JSON_ERR_SUBTOOLONG)?"too many array elements": \
                            (rc==JSON_ERR_BADSUBTRAIL)?"garbage while expecting array comma": \
                            (rc==JSON_ERR_BADSTRING)?"error while string parsing ": \
                            (rc==JSON_ERR_SUBTYPE)?"unsupported array element type": \
                            (rc==JSON_ERR_BADSTRING)?"error while string parsing": \
                            (rc==JSON_ERR_CHECKFAIL)?"check attribute not matched": \
                            (rc==JSON_ERR_NOPARSTR)?"can't support strings in parallel arrays": \
                            (rc==JSON_ERR_BADENUM)?"invalid enumerated value": \
                            (rc==JSON_ERR_QNONSTRING)?"saw quoted value when expecting nonstring 1": \
                            (rc==JSON_ERR_NONQSTRING)?"didn't see quoted value when expecting string": \
                            (rc==JSON_ERR_MISC)?"other data conversion error ": \
                            (rc==JSON_ERR_BADNUM)?"saw quoted value when expecting nonstring 2": \
                            (rc==JSON_ERR_NULLPTR)?"unexpected null value or attribute pointer":"JSON OK")
