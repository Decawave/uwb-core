...
static struct dpl_task s_task[2];
...
{
    int err;
    err = dpl_task_init(NULL,
		       "Null task",
		       NULL, NULL, 1, 0, NULL, 0);
    VerifyOrQuit(err, "dpl_task_init accepted NULL parameters.");

    err = dpl_task_init(&s_task[0],
		       "s_task[0]",
		       task0_run, &s_task_arg[0], 1, 0, NULL, 0);
    SuccessOrQuit(err, "dpl_task_init failed.");

    err = dpl_task_init(&s_task[1],
		       "s_task[1]",
...
{
    int ret = PASS;
    SuccessOrQuit(test_init(),    "Failed: dpl_task_init");
