...
uart_poller(void *arg)
{
    ...
    while (1) {
        ...
        dpl_time_delay(DPL_TICKS_PER_SEC / 100);
    }
    return 0;
}
...
uart_pty(int port)
{
    ...
    /* openpty doesn't exist prior to android_api 23 */
    return -1;
#else
    int fd, rc;
    int loop_slave;
...
hal_uart_blocking_tx(int port, uint8_t data)
{
    ...
    if (uarts[port].u_fd > -1) {
        int rc = write(uarts[port].u_fd, &data, sizeof(data));
        rc = rc;
    }
}
  ...
  hal_uart_rx_char rx_func, void *arg)
{
    ...
    if (!uart_poller_running) {
        ...
        pthread_create(&thread_uart, NULL, uart_poller, NULL);
    }
    return 0;
}
