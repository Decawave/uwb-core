...
/* CPUTIME data. */
struct dpl_cputime_data
{
    uint32_t ticks_per_usec;    /* number of ticks per usec */
};
extern struct dpl_cputime_data g_dpl_cputime;
#endif
 ...
 */
uint32_t dpl_cputime_usecs_to_ticks(uint32_t usecs);
 ...
 */
uint32_t dpl_cputime_ticks_to_usecs(uint32_t ticks);
#endif
