...
struct dpl_mbuf_pool {
     ...
     */
    struct dpl_mempool *omp_pool;

    STAILQ_ENTRY(dpl_mbuf_pool) omp_next;
};
...
struct dpl_mbuf_pkthdr {
    ...
    STAILQ_ENTRY(dpl_mbuf_pkthdr) omp_next;
};
...
struct dpl_mbuf {
     ...
     */
    struct dpl_mbuf_pool *om_omp;

    SLIST_ENTRY(dpl_mbuf) om_next;
 ...
 */
struct dpl_mqueue {
    STAILQ_HEAD(, dpl_mbuf_pkthdr) mq_head;
    /** Event to post when new buffers are available on the queue. */
    struct dpl_event mq_ev;
    struct dpl_mutex mutex;
};
 ...
 */
#define DPL_MBUF_F_MASK(__n) (1 << (__n))
 ...
 */
#define DPL_MBUF_IS_PKTHDR(__om) \
    ((__om)->om_pkthdr_len >= sizeof (struct dpl_mbuf_pkthdr))

/** Get a packet header pointer given an mbuf pointer */
#define DPL_MBUF_PKTHDR(__om) ((struct dpl_mbuf_pkthdr *)     \
    ((uint8_t *)&(__om)->om_data + sizeof(struct dpl_mbuf)))

/** Given a mbuf packet header pointer, return a pointer to the mbuf */
#define DPL_MBUF_PKTHDR_TO_MBUF(__hdr)   \
     (struct dpl_mbuf *)((uint8_t *)(__hdr) - sizeof(struct dpl_mbuf))

/**
 * Gets the length of an entire mbuf chain.  The specified mbuf must have a
 * packet header.
 */
#define DPL_MBUF_PKTLEN(__om) (DPL_MBUF_PKTHDR(__om)->omp_len)
 ...
 */
#define DPL_MBUF_DATA(__om, __type) \
     (__type) ((__om)->om_data)
 ...
 */
#define DPL_MBUF_USRHDR(om)                              \
    (void *)((uint8_t *)om + sizeof (struct dpl_mbuf) +  \
             sizeof (struct dpl_mbuf_pkthdr))
...
_dpl_mbuf_leadingspace(struct dpl_mbuf *om)
{
    uint16_t startoff;
 ...
 */
#define DPL_MBUF_LEADINGSPACE(__om) _dpl_mbuf_leadingspace(__om)
 ...
 */
#define DPL_MBUF_TRAILINGSPACE(__om) _dpl_mbuf_trailingspace(__om)
 ...
 */
int dpl_mqueue_init(struct dpl_mqueue *mq, dpl_event_fn *ev_cb, void *arg);
 ...
 */
struct dpl_mbuf *dpl_mqueue_get(struct dpl_mqueue *);
 ...
 * @return 0 on success, non-zero on failure.
 */
int dpl_mqueue_put(struct dpl_mqueue *, struct dpl_eventq *, struct dpl_mbuf *);
 ...
 * @return 0 on success, non-zero on failure
 */
int dpl_msys_register(struct dpl_mbuf_pool *);
 ...
 */
struct dpl_mbuf *dpl_msys_get(uint16_t dsize, uint16_t leadingspace);

/**
 * De-registers all mbuf pools from msys.
 */
void dpl_msys_reset(void);
 ...
 * @return A freshly allocated mbuf on success, NULL on failure.
 */
struct dpl_mbuf *dpl_msys_get_pkthdr(uint16_t dsize, uint16_t user_hdr_len);
 ...
 */
int dpl_mbuf_pool_init(struct dpl_mbuf_pool *, struct dpl_mempool *mp,
        uint16_t, uint16_t);
 ...
 */
struct dpl_mbuf *dpl_mbuf_get(struct dpl_mbuf_pool *omp, uint16_t);
 ...
 * @return A freshly allocated mbuf on success, NULL on failure.
 */
struct dpl_mbuf *dpl_mbuf_get_pkthdr(struct dpl_mbuf_pool *omp,
        uint8_t pkthdr_len);
 ...
 */
struct dpl_mbuf *dpl_mbuf_dup(struct dpl_mbuf *m);
 ...
 */
struct dpl_mbuf *dpl_mbuf_off(const struct dpl_mbuf *om, int off,
                            uint16_t *out_off);
 ...
 */
int dpl_mbuf_copydata(const struct dpl_mbuf *m, int off, int len, void *dst);
 ...
 */
int dpl_mbuf_append(struct dpl_mbuf *m, const void *, uint16_t);
 ...
 */
int dpl_mbuf_appendfrom(struct dpl_mbuf *dst, const struct dpl_mbuf *src,
 ...
 */
int dpl_mbuf_free(struct dpl_mbuf *mb);
 ...
 * @return 0 on success, -1 on failure
 */
int dpl_mbuf_free_chain(struct dpl_mbuf *om);
 ...
 *                tail of the mbuf.
 */
void dpl_mbuf_adj(struct dpl_mbuf *mp, int req_len);
 ...
 */
int dpl_mbuf_cmpf(const struct dpl_mbuf *om, int off, const void *data, int len);
 ...
 */
int dpl_mbuf_cmpm(const struct dpl_mbuf *om1, uint16_t offset1,
 ...
 *                              NULL on failure.
 */
struct dpl_mbuf *dpl_mbuf_prepend(struct dpl_mbuf *om, int len);
 ...
 */
struct dpl_mbuf *dpl_mbuf_prepend_pullup(struct dpl_mbuf *om, uint16_t len);
 ...
 */
int dpl_mbuf_copyinto(struct dpl_mbuf *om, int off, const void *src, int len);
 ...
 */
void dpl_mbuf_concat(struct dpl_mbuf *first, struct dpl_mbuf *second);
 ...
 *                              NULL on failure.
 */
void *dpl_mbuf_extend(struct dpl_mbuf *om, uint16_t len);
 ...
 */
struct dpl_mbuf *dpl_mbuf_pullup(struct dpl_mbuf *om, uint16_t len);
 ...
 */
struct dpl_mbuf *dpl_mbuf_trim_front(struct dpl_mbuf *om);
