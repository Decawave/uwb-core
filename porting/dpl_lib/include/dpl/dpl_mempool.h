...
/**
 * @addtogroup DPLKernel
 * @{
 *   @defgroup DPLMempool Memory Pools
 *   @{
 */


#ifndef _DPL_MEMPOOL_H_
#define _DPL_MEMPOOL_H_
 ...
 */
struct dpl_memblock {
    SLIST_ENTRY(dpl_memblock) mb_next;
};
...
/* XXX: Change how I coded the SLIST_HEAD here. It should be named:
   SLIST_HEAD(,dpl_memblock) mp_head; */
...
struct dpl_mempool {
    ...
    STAILQ_ENTRY(dpl_mempool) mp_list;
    SLIST_HEAD(,dpl_memblock);
 ...
 */
typedef dpl_error_t dpl_mempool_put_fn(struct dpl_mempool_ext *ome, void *data,
                                     void *arg);

struct dpl_mempool_ext {
    struct dpl_mempool mpe_mp;

    /* Callback that is executed immediately when a block is freed. */
    dpl_mempool_put_fn *mpe_put_cb;
    void *mpe_put_arg;
};

#define DPL_MEMPOOL_INFO_NAME_LEN (32)

/**
 * Information describing a memory pool, used to return OS information
 * to the management layer.
 */
struct dpl_mempool_info {
    ...
    char omi_name[DPL_MEMPOOL_INFO_NAME_LEN];
};
 ...
 */
struct dpl_mempool *dpl_mempool_info_get_next(struct dpl_mempool *,
        struct dpl_mempool_info *);
 ...
 * the memory pool.
 */
#if (DPL_CFG_ALIGNMENT == OS_CFG_ALIGN_4)
#define DPL_MEMPOOL_SIZE(n,blksize)      ((((blksize) + 3) / 4) * (n))
typedef uint32_t dpl_membuf_t;
#else
#define DPL_MEMPOOL_SIZE(n,blksize)      ((((blksize) + 7) / 8) * (n))
typedef uint64_t dpl_membuf_t;
#endif

/** Calculates the number of bytes required to initialize a memory pool. */
#define DPL_MEMPOOL_BYTES(n,blksize)     \
    (sizeof (dpl_membuf_t) * DPL_MEMPOOL_SIZE((n), (blksize)))
 ...
 * @return dpl_error_t
 */
dpl_error_t dpl_mempool_init(struct dpl_mempool *mp, uint16_t blocks,
 ...
 * @return dpl_error_t
 */
dpl_error_t dpl_mempool_ext_init(struct dpl_mempool_ext *mpe, uint16_t blocks,
                               uint32_t block_size, void *membuf, char *name);
 ...
 * @return dpl_error_t
 */
dpl_error_t dpl_mempool_clear(struct dpl_mempool *mp);
 ...
 */
bool dpl_mempool_is_sane(const struct dpl_mempool *mp);
 ...
 */
int dpl_memblock_from(const struct dpl_mempool *mp, const void *block_addr);
 ...
 */
void *dpl_memblock_get(struct dpl_mempool *mp);
 ...
 * @return dpl_error_t
 */
dpl_error_t dpl_memblock_put_from_cb(struct dpl_mempool *mp, void *block_addr);
 ...
 * @return dpl_error_t
 */
dpl_error_t dpl_memblock_put(struct dpl_mempool *mp, void *block_addr);

#ifdef __cplusplus
}
#endif

#endif  /* _DPL_MEMPOOL_H_ */
