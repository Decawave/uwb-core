                       ...
                       void **out_buf)
{
    ...
    if (num_blocks > 0) {
        *out_buf = malloc(DPL_MEMPOOL_BYTES(num_blocks, block_size));
        if (*out_buf == NULL) {
            return DPL_ENOMEM;
        }
    } else {
                   ...
                   uint32_t block_size, char *name, void **out_buf)
{
    ...
    rc = dpl_mempool_init(mempool, num_blocks, block_size, buf, name);
    if (rc != 0) {
        free(buf);
        return rc;
    }
                       ...
                       uint32_t block_size, char *name, void **out_buf)
{
    ...
    rc = dpl_mempool_ext_init(mpe, num_blocks, block_size, buf, name);
    if (rc != 0) {
        free(buf);
        return rc;
    }
