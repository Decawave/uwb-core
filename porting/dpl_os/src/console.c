...
static struct dpl_event shell_console_ev[CONSOLE_COMPAT_MAX_CMD_QUEUED];
static console_rx_cb console_compat_rx_cb; /* callback that input is ready */
static struct dpl_eventq compat_avail_queue;
static struct dpl_eventq compat_lines_queue;
#endif
...
static struct dpl_eventq avail_queue;
static struct dpl_eventq *lines_queue;
static completion_cb completion;
bool g_console_silence;
bool g_console_silence_non_nlip;
bool g_console_ignore_non_nlip;
static struct dpl_mutex console_write_lock;
...
console_read(char *str, int cnt, int *newline)
{
    ...
    *newline = 0;
    ev = dpl_eventq_get_no_wait(lines_queue);
    if (!ev) {
        return 0;
    }
    cmd = ev->ev_arg;
...
console_handle_char(uint8_t byte)
{
    ...
    if (!ev) {
        ev = dpl_eventq_get_no_wait(&avail_queue);
        if (!ev) {
            rx_stalled = true;
            return -1;
        }
...
void
console_line_queue_set(struct dpl_eventq *evq)
{
    lines_queue = evq;
}

void
console_line_event_put(struct dpl_event *ev)
{
    dpl_eventq_put(&avail_queue, ev);
...
console_init(console_rx_cb rx_cb)
{
    ...
    dpl_eventq_init(&compat_lines_queue);
    dpl_eventq_init(&compat_avail_queue);
...
console_pkg_init(void)
{
    ...
    dpl_eventq_init(&avail_queue);
    dpl_mutex_init(&console_write_lock);
